  ...
  return (
    <div className="relative z-10 max-w-5xl mx-auto px-4 md:px-10 py-8">
      ...
      <div className="mb-6">
        ...
        <p className="text-[#6b7494] text-sm mb-4">
          ReportAgent synthesized all simulation data into a structured prediction.
        </p>
        <button onClick={downloadReport}
          className="w-full md:w-auto inline-flex items-center justify-center gap-2 px-5 py-2.5 border border-[#00e5ff]/30 text-[#00e5ff] font-mono text-sm rounded-xl hover:bg-[#00e5ff]/10 transition-all">
          ...
          {REPORT_SECTIONS.map((s, i) => (
            <div key={i}
              ...
              className={`px-3.5 py-2.5 rounded-lg cursor-pointer text-sm transition-all mb-1 border
                ${activeIdx === i
                  ? 'bg-[#00e5ff]/8 text-[#00e5ff] border-[#00e5ff]/20'
                  : 'text-[#6b7494] border-transparent hover:text-[#e8eaf0]'}`}>
              {s.title}
            </div>
          ))}
        </div>
      ...
      <div className="md:hidden flex flex-col gap-3">
        {REPORT_SECTIONS.map((s, i) => <Section key={i} s={s} i={i} />)}
      </div>
