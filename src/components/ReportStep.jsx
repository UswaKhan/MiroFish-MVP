    ...
    <div className="relative z-10 max-w-5xl mx-auto px-4 md:px-10 py-8">
      ...
      <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4 mb-6">
        <div>
          ...
          <h2 className="font-extrabold text-2xl md:text-3xl tracking-tight mb-2" style={{fontFamily:'Syne,sans-serif'}}>Analysis Complete</h2>
          <p className="text-[#6b7494] text-sm">ReportAgent synthesized all simulation data into a structured prediction.</p>
        </div>
        <button onClick={downloadReport}
          className="w-full md:w-auto px-5 py-2.5 border border-[#00e5ff]/30 text-[#00e5ff] font-mono text-sm rounded-xl hover:bg-[#00e5ff]/10 transition-all text-center">
          ⬇ Download Report
        </button>
      </div>
      ...
      {/* Mobile — no TOC, just sections */}
      <div className="md:hidden flex flex-col gap-4">
        {REPORT_SECTIONS.map((s, i) => (
          <div key={i} className="bg-[#0f1117] border border-[#1e2535] rounded-2xl p-5 opacity-0"
               style={{ animation: `fadeIn 0.5s ease ${i * 0.13}s forwards` }}>
            <h3 className="font-extrabold text-base mb-3 pb-3 border-b border-[#1e2535]" style={{fontFamily:'Syne,sans-serif'}}>{s.title}</h3>
            {s.paragraphs.map((p, j) => <p key={j} className="text-[#6b7494] text-sm leading-relaxed mb-3">{p}</p>)}
            {s.showSentiment && (
              <div className="my-4">
                <div className="flex h-2.5 rounded-full overflow-hidden mb-3">
      ...
      <div className="mt-6">
        <button onClick={onNext}
          className="w-full md:w-auto md:float-right px-8 py-3.5 bg-[#00e5ff] text-black font-mono font-bold text-sm rounded-xl hover:brightness-110 transition-all">
